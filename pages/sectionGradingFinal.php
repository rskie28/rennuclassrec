...
<?php
	...
	if (isset($_POST['btnUpdatePerc'])) {
		...
		$sqlUpdatePerc = "UPDATE gradeperc SET SWPerc = ".$sw.", EXPerc = ".$ex.", HWPerc = ".$hw.", QZPerc = ".$qz.", OTHPerc = ".$oth.", PTPerc = ".$pt." WHERE sectionID = '".$_SESSION['id']."' AND term = 'F'";
		...
		if(($sw+$ex+$hw+$qz+$oth+$pt != 100)) {
			...
			if($con->query($sqlUpdatePerc)==TRUE) {
				...
				Print '<script>window.location.assign("sectionGradingFinal.php?id='.$_SESSION['id'].'"); </script>';
			} else {
